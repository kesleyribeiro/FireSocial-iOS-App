...
public extension UITextField {
    ...
    @IBOutlet weak var nextTextField: UITextField? {
        get {
            return retrieveTextField(key: &NextTextFieldKey)
        }

        set {
            nextTextField?.previousTextField = nil
            storeTextField(textField: newValue, withKey: &NextTextFieldKey)
    ...
    weak internal(set) var previousTextField: UITextField? {
        get {
            return retrieveTextField(key: &PreviousTextFieldKey)
        }

        set {
            storeTextField(textField: newValue, withKey: &PreviousTextFieldKey)
            applyInputAccessoryView()
        }
    }
    ...
    internal func storeTextField(textField: UITextField?, withKey key: UnsafeRawPointer) {
    ...
    internal func retrieveTextField(key: UnsafeRawPointer) -> UITextField? {
        ...
        guard let textField = (weakObjectContainer as? WeakObjectContainer)?.object as? UITextField else {
            storeTextField(textField: nil, withKey: key)
            return nil
        }
        return textField
    }
